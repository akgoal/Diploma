...
public class UserViewImpl extends Composite implements UserView, ClickHandler {
    ...
    public UserViewImpl() {
                        ...
                        new AsyncCallback<Boolean>() {
                            public void onFailure(Throwable caught) {
                                if (caught instanceof UserEmailNotVerifiedException) {
                                    Window.alert("Email пользователя не подтвержден...");
                                } else {
                                    Window.alert("Ошибка!!!");
                                }
                            }
                            ...
                            public void onSuccess(Boolean auth_result) {
                                if (auth_result) {
                                    ...
                                    showUserAuthenticatedForm(username.getText());
                                } else {
                                    username.setText("");
                                    password.setText("");
                                }
        ...
        if (SecurityUtils.getAuthenticatedUsername() != null) {
            showUserAuthenticatedForm(SecurityUtils.getAuthenticatedUsername());
        }
    ...
    private void showUserAuthenticatedForm(String username) {
        form_auth.clear();
    ...
    public void setViewReg() {
        ...
        hPanel.clear();
        titlePanel.clear();
        Reg reg = new Reg(new Reg.OnRegEvent() {
            @Override
                ...
                securityService.register(userDto, new AsyncCallback<Void>() {
                    ...
                    public void onFailure(Throwable throwable) {
                        if (throwable instanceof UsernameAlreadyExistsException) {
                            Window.alert("Ошибка регистрации. Логин занят");
                        } else if (throwable instanceof EmailAlreadyExistsException) {
                            Window.alert("Ошибка регистрации. Email занят");
                        }
                    }

                    @Override
                    public void onSuccess(Void aVoid) {
                        Window.alert("Пользователь успешно зарегистрирован. Вам на почту отправлен email подтвеждения!");
                    }
                });
            }
        });
