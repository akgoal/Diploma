...
public class UserViewImpl extends Composite implements UserView, ClickHandler {
    ...
    public UserViewImpl() {
                ...
                securityService.authenticate(username.getText(), password.getText(),
                        new AsyncCallback<Boolean>() {
                            public void onFailure(Throwable caught) {
                                if (caught instanceof UserEmailNotVerifiedException) {
                                    Window.alert("Email не подтворжденюю");
                                } else {
                                    Window.alert("Не правильный логин/пароль ...");
                                }
                            }

                            public void onSuccess(Boolean auth_result) {
                                if (auth_result) {
                                    showUserForm(username.getText());
                                    SecurityUtils.setAuthenticatedUsername(username.getText());
                                } else {
                                    username.setText("");
                                    password.setText("");
                                }
        ...
        button_outh.addClickHandler(new ClickHandler() {
            ...
            @Override
            public void onClick(ClickEvent event) {
                SecurityUtils.setAuthenticatedUsername(null);
                form_auth.clear();
                form_auth.add(username);
                form_auth.add(password);
                but_panel.add(button_auth);
        ...
        basket.getButton().addClickHandler(new ClickHandler() {
            ...
            @Override
            public void onClick(ClickEvent event) {
                orderService.addOrder(basket.getCheck(),SecurityUtils.getAuthenticatedUsername(), new AsyncCallback<Void>() {
        ...
        if (SecurityUtils.getAuthenticatedUsername() != null) {
            showUserForm(SecurityUtils.getAuthenticatedUsername());
        }
    }

    private void showUserForm(String username) {
        form_auth.clear();
    ...
    private void addBookInBasket() {
        ...
        orderService.listBook(SecurityUtils.getAuthenticatedUsername(),new AsyncCallback<ArrayList<Book>>() {
                ...
                if (result.size() == 0) {
                    ...
                    for (Book book : result) {
                        ...
                        removeBook.addClickHandler(new ClickHandler() {
                            public void onClick(ClickEvent event) {
                                orderService.delBook(index,SecurityUtils.getAuthenticatedUsername(), new AsyncCallback<Void>() {
    ...
    public void setViewReg() {
        ...
        hPanel.clear();
        titlePanel.clear();
        fPanel.add(new Reg(new Reg.OnRegEvent() {
            @Override
            public void onReg(UserDto userDto) {
                securityService.register(userDto, new AsyncCallback<Void>() {
                    @Override
                    public void onFailure(Throwable throwable) {
                        if (throwable instanceof UsernameAlreadyExistsException) {
                            Window.alert("UsernameAlreadyExistsException");
                        } else if (throwable instanceof EmailAlreadyExistsException) {
                            Window.alert("EmailAlreadyExistsException");
                        } else {
                    ...
                    public void onSuccess(Void aVoid) {
                        Window.alert("Пользователь успешно ...");
                    }
                });
            }
        }));
    ...
    public void setViewHistory() {
        ...
        hPanel.clear();
        titlePanel.clear();
        orderService.listOrder(SecurityUtils.getAuthenticatedUsername(), new AsyncCallback<ArrayList<OrderDto>>() {
            ...
            @Override
            public void onSuccess(ArrayList<OrderDto> orders) {
