...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "librarybooks" because\n';
  ...
  $wnd.__gwt_activeModules['librarybooks'] = {
    'moduleName' : 'librarybooks',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/librarybooks.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['librarybooks'].superdevmode = true;

  var expectedSuffix = '/librarybooks.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + librarybooks + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:librarybooks';
  var devModeSessionKey = '__gwtDevModeSession:librarybooks';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/librarybooks/librarybooks.nocache.js';
  ...
  $wnd.__gwt_activeModules['librarybooks'].canRedirect = true;
  $wnd.__gwt_activeModules['librarybooks'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING librarybooks';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/librarybooks?user.agent=' + ua + '&_callback=' + callback;
