...
public class BookServiceDBImpl extends RemoteServiceServlet implements BookService, GreetingService {
	...
	public ArrayList<Book> sendServer() {
		...
		try {
			...
			for (BooksDataSet bds : dataSets)
			{
				books.add(new Book(
						bds.getId(), 
						bds.getAuthor().getName(),
						bds.getAuthor().getId(),
						bds.getTitle(),
						bds.getGenre().getName(),
						bds.getGenre().getId(),
						bds.getImgUrl()
						));
			}
	...
	@Override
	public ArrayList<Book> findBooksByAutorBook(long callInput) {
		ArrayList<Book> books = new ArrayList<>();
		try {
			Session session = sessionFactory.openSession();
			BooksDAO dao = new BooksDAO(session);
